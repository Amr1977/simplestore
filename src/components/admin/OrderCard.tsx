...
const statusMap: Record<string, { label: string; color: string }> = {
  ...
  confirmed: { label: 'مؤكد', color: 'bg-purple-100 text-purple-700' },
...
export default function OrderCard({ order }: { order: Order }) {
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    setUpdating(true)
    await updateOrderStatus(order.storeId, order.id, newStatus)
    setUpdating(false)
  }
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3" dir="rtl">
      ...
      <div className="flex items-center justify-between pt-2 border-t border-gray-100">
        <div className="text-left">
          <p className="text-lg font-bold text-gray-900">{order.total.toFixed(0)} جنيه</p>
